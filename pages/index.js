...
export default function Home() {
  ...
  const getWeather = async ()=>{
    const api_key = '5c60c76746b0448f958171530230309'
  ...
  <div className='text-center text-2xl p-2'>{api_data.city}</div>
            <div className='flex justify-center'>
              <div className='flow-root'>
                ...
                <div className='float-left text-6xl desgrees'>{api_data.temp}</div>
              </div>
            </div>
            <div className='text-center text-gray-500'>{api_data.condition}</div>
            <div className='flow-root p-2'>
              <div className='float-left text-gray-500'>Humidade:{api_data.humidity}%</div>
              <div className='float-right text-gray-500'>Vento:{api_data.wind}mph</div>
              <div className='float-left text-gray-500'>Visibilidade:{api_data.visibility}mph</div>
              <div className='float-right text-gray-500'>Rajado:{api_data.gust}mph</div>
            </div>
  </>)  
      }
  ...
  return (
    ...
    type='text' id='location' onChange={(e)=> setlocation(e.target.value)} placeholder='localização (PR. Paraná)'
    />
    </div>
